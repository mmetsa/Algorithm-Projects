...
/** Testklass.
 * @author jaanus
 * updated by Maiko - added new test cases
 */
public class LongStackTest {
    ...
    @Test (expected=RuntimeException.class)
    public void testSwapUnderflow2() {
        String s = "2 SWAP -";
        LongStack.interpret (s);
    }

    @Test (expected=RuntimeException.class)
    public void testSwapUnderflow3() {
        String s = "2 5 7 + - SWAP -";
        LongStack.interpret (s);
    }
    ...
    @Test (expected=RuntimeException.class)
    public void testRotUnderflow3() {
        String s = "2 5 7 9 + - ROT -";
        LongStack.interpret (s);
    }
    ...
    public void testSwap() {
        String s = " 1 2 SWAP -";
        assertEquals("expression: " + Aout.toString(s), 1, LongStack.interpret(s));
        s = "1 2 SWAP SWAP -";
        assertEquals("expression: " + Aout.toString(s), -1, LongStack.interpret(s));
        s = "1 2 3 SWAP - +";
        assertEquals("expression: " + Aout.toString(s), 2, LongStack.interpret(s));
        s = "1 2 SWAP - 3 SWAP -";
        assertEquals("expression: " + Aout.toString(s), 2, LongStack.interpret(s));
    }
    ...
    public void testRot() {
        String s =  "3 6 9 ROT - -";
        assertEquals("expression: " + Aout.toString(s), 0, LongStack.interpret(s));
        s =  "3 6 9 ROT - 5 ROT + + 1 1 ROT + +";
        assertEquals("expression: " + Aout.toString(s), 19, LongStack.interpret(s));
        s =  "1 2 3 ROT - /";
        assertEquals("expression: " + Aout.toString(s), 1, LongStack.interpret(s));
        s =  "2 4 6 ROT / 2 ROT - -";
        assertEquals("expression: " + Aout.toString(s), 5, LongStack.interpret(s));
        s =  "3 6 9 ROT - - 17 23 ROT - - 33 65 ROT - -";
        assertEquals("expression: " + Aout.toString(s), -38, LongStack.interpret(s));
    }

    @Test (timeout = 1000)
    public void testRotAndSwap() {
        String s = "1 2 3 ROT - +";
        assertEquals("expression: " + Aout.toString(s), 4, LongStack.interpret(s));
        s = "2 5 9 ROT - +";
        assertEquals("expression: " + Aout.toString(s), 12, LongStack.interpret(s));
        s = "2 5 9 ROT + SWAP -";
        assertEquals("expression: " + Aout.toString(s), 6, LongStack.interpret(s));
        s = "2 5 SWAP - 9 * 25 SWAP - 2 +";
        assertEquals("expression: " + Aout.toString(s), 0, LongStack.interpret(s));
        s = "3 6 SWAP / 2 - 3 6 ROT - SWAP /";
        assertEquals("expression: " + Aout.toString(s), 2, LongStack.interpret(s));
    }
